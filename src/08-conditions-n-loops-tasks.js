...
function getSumBetweenNumbers(a, b) {
  if (a !== b) return a + getSumBetweenNumbers(a + 1, b);
  return b;
}
...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (left <= l && top <= t && left + width > l && top + height > t) return true;
  if (l <= left && t <= top && l + w > left && t + h > top) return true;
  return false;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  let [, , start, end] = [a, b, isStartIncluded, isEndIncluded];
  if (start === true) start = '[';
  else start = '(';
  if (end === true) end = ']';
  else end = ')';
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < config.length;) {
    if (res.includes(config[i]) === true) {
      res = res.replace(config[i], '');
      i = 0;
    } else {
      i += 1;
    }
  }
  if (res.length === 0) res = true;
  else res = false;
  return res;
}
